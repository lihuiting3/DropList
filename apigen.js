...
var SECURITY_LEVEL = ['User', 'RegisteredDevice', 'Anonym'];
var SECURITY_TYPE = {
  'User': 'SecurityType.UserLogin',
  'RegisteredDevice': 'SecurityType.RegisteredDevice',
  'Anonym': 'SecurityType.None'
};
...
var SOURCE = [
  ...
  // },
  {
    name: 'yit',
    //src: 'http://10.32.156.47/info.api?json',
    // src: 'http://cms.stage.yit.com/info.cmsapi?json',
    src: 'http://m.api.i3.yit.com/apigw/info.api?json',
    filename: 'yit.json',
    // filterGroup: ['integral','order', 'recommend', 'minicart', 'user', 'logistics', 'product', 'shopcart', 'b2cmall', 'sc', 'supplychain', 'coupon', 'cp', 'promotion', 'payment','categoryPage','finance']
    filterGroup: ['seriesSpu','cms']
  }
];
...
function createAPI(grunt, done) {
  ...
  grunt.file.recurse(ROOT_PATH + '/source/', function(abspath, rootdir, subdir, file) {
    ...
    for (var i in json.apiList) {
      ...
      grunt.log.ok(it.groupName + ":" + it.securityLevel);
